/**
 * Subway Builder Modding API.
 *
 * Base model ported from Improved Schematics (API v1.0.0) and re-verified
 * against the shipped Electron build (`dist/renderer/public/index-*.js`) while
 * building the Induced Demand mod. Members added or corrected during that pass
 * are marked `@added` / `@verified` in their doc comments.
 *
...
import type { GameSchemas, BuildingsIndex } from './schemas';

// =============================================================================
// HELPER TYPES
// =============================================================================

/** Options accepted by `gameState.getStations()` / `getRoutes()`. */
export interface GetEntitiesOptions {
  /**
   * Include in-progress (temp-parent) routes. When `false`, temp routes are
   * omitted from `getRoutes()` and filtered out of each station's `routeIds`.
   * @default true
   */
  includeTempRoutes?: boolean;
}
...
export interface ModdingAPI {
  ...
  gameState: {
    /**
     * Get all stations.
     * @param options.includeTempRoutes When `false`, each station's `routeIds`
     *   is filtered to exclude in-progress (temp-parent) routes. Defaults to
     *   `true`. (Option name inferred from the minified build.)
     */
    getStations(options?: GetEntitiesOptions): Station[];
    /**
     * Get all routes.
     * @param options.includeTempRoutes When `false`, in-progress (temp-parent)
     *   routes are omitted. Defaults to `true`. (Inferred from the build.)
     */
    getRoutes(options?: GetEntitiesOptions): Route[];
    /** Get all tracks. */
    getTracks(): Track[];
    /** Get all trains. */
    getTrains(): Train[];
     ...
     *
     * @remarks
     * Returns the **live** store object — the same `points` and `popsMap` `Map`
     * instances the simulation reads at the start of every commute cycle. There
     * is no public demand setter; the supported way to *change* demand is to
     * mutate the returned Maps in place (adjust `DemandPoint.residents`/`.jobs`,
     * scale or add `Pop` entries). Mutations are picked up by the next
     * `simulateCommutes` pass with no further call required.
     *
     * Caveat: each commute pass replaces `demandData` with freshly-derived
     * `points`/`popsMap`, so re-fetch via `getDemandData()` rather than caching
     * the object across cycles. See `docs/DEMAND_API.md`.
     */
    ...
    /**
     * Proximity-merged station interchange groups (same data as the in-game
     * schematic). Returns `[]` when no groups exist.
     * @verified Now a concrete method returning `StationGroup[]`.
     */
    getStationGroups(): StationGroup[];
    /**
     * IDs of all stations that belong to an interchange group of 2+ stations
     * (i.e. transfer stations).
     * @added Present in the shipped build; was missing from the v1.0.0 model.
     */
    getTransferStationIds(): string[];
    /**
     * IDs of the other stations grouped with `stationId` (its transfer
     * siblings), or `[]` if it is not part of a multi-station group.
     * @added
     */
    getSiblingStationIds(stationId: string): string[];
    /**
     * The current save's name, or `null` if the game has not been saved.
     * @added
     */
  ...
  utils: {
    ...
    i18n: I18nAPI;
    /**
     * Building-index helpers (binary encode/validate/load).
     * @added Present in the shipped build; was missing from the v1.0.0 model.
     */
    buildings: {
      /** Encode a buildings index to the binary `.sbbi` format. */
      encode(index: BuildingsIndex): ArrayBuffer;
      /** Validate a binary buildings-index buffer. */
      validate(buffer: ArrayBuffer):
        | { ok: true; buildingCount: number; nonEmptyCells: number; bbox: [number, number, number, number] }
        | { ok: false; error: string };
      /** Load (decode) a binary buildings index from a path/URL. */
      load(path: string): Promise<BuildingsIndex>;
    };
  };
