    ...
    if (heatKey !== lastHeatKey) { // re-bake only when content changed
      ...
      if (view === 'accessRes' || view === 'accessCom') {
        ...
        raster = rasterizeAccessField(bbox, (lon, lat) => {
          // Show access with the connectivity FLOOR removed (walkProx × Ô): keep
          // the distance taper but drop the constant 0.3 floor, so each station's
          // proximity blob is scaled by how well-connected it is.
          const a = f.accessIdx.accessNoFloorAt([lon, lat]);
