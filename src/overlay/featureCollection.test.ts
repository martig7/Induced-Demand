...
const NO_ACCESS = () => ({ res: 0, com: 0 });
const FULL_ACCESS = () => ({ res: 0.8, com: 0.8 });
...
test('realized counts induced pop sizes anchored at each point (combined = residence + job)', () => {
  ...
  const fc = buildOverlay(dd, NO_ACCESS, 'realized', 'combined', DEFAULT_CONFIG);
...
test('realized residential vs commercial split by anchor side', () => {
  ...
  const res = buildOverlay(dd, NO_ACCESS, 'realized', 'residential', DEFAULT_CONFIG);
  ...
  const com = buildOverlay(dd, NO_ACCESS, 'realized', 'commercial', DEFAULT_CONFIG);
...
test('targeting uses the model score (point with injected access)', () => {
  const dd: DemandData = { points: new Map([['H', pt('H', [0, 0], 400, 0, 50, 0)]]), popsMap: new Map() };
  const fc = buildOverlay(dd, FULL_ACCESS, 'targeting', 'residential', DEFAULT_CONFIG);
  assert.equal(fc.features.length, 1);
  // access-dominant score: access(0.8) × (0.5 + 0.5×0.5) = 0.6
  assert.ok(Math.abs(fc.features[0].properties.value - 0.6) < 1e-6);
});

test('targeting drops points with zero access', () => {
  const dd: DemandData = { points: new Map([['H', pt('H', [0, 0], 400, 0, 50, 0)]]), popsMap: new Map() };
  const fc = buildOverlay(dd, NO_ACCESS, 'targeting', 'residential', DEFAULT_CONFIG);
  assert.equal(fc.features.length, 0);
});
...
test('normalization sets t = value / maxValue across points', () => {
  ...
  const fc = buildOverlay(dd, NO_ACCESS, 'realized', 'residential', DEFAULT_CONFIG);
